...
def run_exp():
    print("#######################################################################")
    print("#####################  OPTIPIM EXPERIMENT Fig14  ######################")
    print("#######################################################################")
    print()
    ...
    for nn_model in nn_models_list:
        for device_type in target_device_list:
            ...
            if (nn_model != "bert"):
                ...
                sample_layer_folder = f"{NN_MODELS_PATH}/{nn_model}/single_layers/128banks_full"
            
            # Get all layers in the folder
            layers_to_run = [file for file in os.listdir(sample_layer_folder)]
            
            #
            model_folder_path = f"{NN_MODELS_PATH}/{nn_model}/single_layers/holistic_exp"
            ...
            for sel_layer in layers_to_run:
                for num_banks in num_banks_list:
                    ...
                    folder_path = f"{PROJECT_ROOT}/exp_results/fig14/{nn_model}/{target_device_idx_name_map[device_type]}/{layer_name}"
...
def parse_results():
    print("\n[INFO] Extract Results for Fig14!")
    ...
    for nn_model in nn_models_list:
        ...
        for device_type in target_device_list:
            ...
            result_folder_base = f"{PROJECT_ROOT}/exp_results/fig14/{nn_model}/{target_device_idx_name_map[device_type]}"
    ...
    json_file_path = f"{PROJECT_ROOT}/exp_results/fig14/all_layer_results_ae.json"
    with open(json_file_path, 'w') as f:
        json.dump(json_data, f, indent=4)
    print(f"[INFO] Generated: {json_file_path}")
